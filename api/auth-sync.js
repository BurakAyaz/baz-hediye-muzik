...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // Kullanıcıyı bul - Cache'i atlamak için her zaman fresh data çek
        ...
        // PlanId'nin doğru değerini al - veritabanındaki değeri kullan
        const userPlan = user.planId || 'none';
        
        // Süre kontrolü - Eğer expiresAt geçmişse krediyi sıfırla
        const now = new Date();
        let creditsToReturn = user.credits || 0;
        let needsUpdate = false;
        
        if (user.expiresAt && new Date(user.expiresAt) < now && user.credits > 0) {
            // Süre dolmuş ve hala kredi var - sıfırla
            needsUpdate = true;
            creditsToReturn = 0;
            
            console.log('⚠️ Süre dolmuş kullanıcı tespit edildi:', user.wixUserId, 'Kredi sıfırlanıyor:', user.credits, '→ 0');
        ...
        console.log('Auth sync - User:', user.wixUserId, 'Plan:', userPlan, 'Credits:', creditsToReturn, 'Expired:', needsUpdate);
        ...
        return res.status(200).json({
            ...
            user: {
                ...
                plan: userPlan,  // Plan bilgisini tutarlı şekilde gönder
                planId: userPlan,  // Plan bilgisini tutarlı şekilde gönder
                credits: creditsToReturn,  // Sıfırlanmış kredi miktarı
                available: creditsToReturn,  // Sıfırlanmış kredi miktarı
